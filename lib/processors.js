...
function updateSecurity(method, headerName) {
  ...
  if (
    method.security.map((s) => Object.keys(s)[0]).indexOf(headerName) === -1
  ) {
    const obj = {};
    obj[headerName] = [];
...
module.exports.processHeaders = (req, method, spec) => {
  Object.keys(req.headers)
    .filter(
      (h) =>
        h.toLowerCase() === "authorization" || h.toLowerCase().startsWith("x-")
    )
    .forEach((h) => {
...
function updateResponses(res, method, chunks) {
  ...
  if (
    !contentType ||
    (contentType.indexOf("json") === -1 && contentType.indexOf("text") === -1)
  ) {
    return;
  }
...
function isCompressed(res) {
  ...
  return (
    ["gzip", "compress", "deflate"].indexOf(headers["content-encoding"]) !== -1
  );
}
    ...
    try {
      if (!isCompressed(res)) {
        ...
        if (
          !method.responses ||
          (!method.responses[res.statusCode] &&
            ![500, 418].includes(res.statusCode))
        ) {
